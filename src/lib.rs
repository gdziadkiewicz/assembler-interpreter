use model::{
    AssemblerInterpreterError, Instruction,
};
use runtime::interpret_instructions;
...
pub fn interpret(input: &str) -> Option<String> {
    interpret_to_result(input).unwrap()
}
...
pub mod tests {
    ...
    "#;
        ...
        assert!(interpret_to_result(program).as_ref() == Ok(&expected));
    }
